class AuctionBidder
  ...
  def self.perform(auction_id)
    ...
    if @auction.auction_status != "Deleted"
      ...
      # Sleeps for the time remaining in the auction, and subtracts two more seconds just for good measure.
      @sleep_time = Time.now - Time.parse(@auction.item[:get_item_response][:item][:listing_details][:end_time]).localtime - 2 - @auction.lead_time
      unless @sleep_time < 0
        sleep(@sleep_time)
      end
...
def test_enqueue_time
  @auction = Auction.first
  puts "Auction ends at #{Time.parse(@auction.item[:get_item_response][:item][:listing_details][:end_time]).localtime}"
  puts "AuctionBidder is being enqueued at #{Time.now + @auction.get_enqueue_time.seconds}"
  puts "Bidding at #{Time.now + (Time.parse(@auction.item[:get_item_response][:item][:listing_details][:end_time]).localtime - Time.now - 2 - @auction.lead_time).seconds}"
end